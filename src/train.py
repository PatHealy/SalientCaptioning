...
# Originally authored by Jason Brownlee
# Adapted by Pat Healy
...
def evaluate_model(model, descriptions, photos, tokenizer, max_length, save_trial=False, save_name=''):
	actual, predicted = list(), list()
	to_save_actual, to_save_predicted = list(), list()
	...
	for key, desc in descriptions.items():
		...
		if save_trial:
			to_save_actual.append(desc)
			to_save_predicted.append(yhat)
	if save_trial:
		df = DataFrame(data={'actual': to_save_actual, 'predicted': to_save_predicted})
...
# train-test split
train = load_set('train.txt')
test = load_set('test.txt')
# descriptions
train_descriptions = load_clean_descriptions('train.txt')
test_descriptions = load_clean_descriptions('test.txt')
...
# define experiment
model_name = 'nontruncated'
verbose = 1
n_epochs = 50
n_photos_per_update = 3
...
for i in range(n_repeats):
	...
	test_score = evaluate_model(model, test_descriptions, test_features, tokenizer, max_length, save_trial=True, save_name=model_name + "_generated_" + str(i) + ".csv")
	# store
...
print("Results saved to " + model_name + ".csv")

model.save(model_name)
print()
print("Model saved to " + str(model_name))